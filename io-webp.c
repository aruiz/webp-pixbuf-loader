...
static gboolean gdk_pixbuf__webp_image_load_increment(gpointer data,
                                                      const guchar *buf,
                                                      guint size,
                                                      GError **error) {
  if (error) {
  ...
  if (context->got_header == FALSE) {
    ...
    context->pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, features.has_alpha, 8,
                                     width, height);
    ...
    context->deccfg.output.colorspace =
    ...
    context->deccfg.output.u.RGBA.stride =
  ...
  context->update_func(context->pixbuf, 0, 0, width, last_y,
                       context->user_data);
